...
\begin{tikzpicture}
  ...
  \node[font=\fontsize{40}{46}\selectfont\bfseries, text=hcGold] at (2.75,3.2) {\$3.4T};
  \node[font=\large\bfseries, text=hcText] at (2.75,2.2) {SFR Market};
  \node[font=\small, text=hcMuted, align=center] at (2.75,1.0) {Institutional capital flooding in.\\Operators need forecasts to compete.};
\end{tikzpicture}
\end{center}
\end{column}
\begin{column}{0.48\textwidth}
...
\begin{tikzpicture}
  \fill[hcCard, rounded corners=8pt] (0,0) rectangle (5.5,4.5);
  \node[font=\fontsize{30}{36}\selectfont\bfseries, text=hcGold, align=center] at (2.75,3.2) {Diffusion\\Models};
  \node[font=\large\bfseries, text=hcText] at (2.75,2.2) {AI Inflection};
  \node[font=\small, text=hcMuted, align=center] at (2.75,1.0) {Foundation models now generate\\probabilistic futures, not point guesses.};
\end{tikzpicture}
\end{center}
\end{column}
\end{columns}
