% Homecastr Investor Pitch Deck - V3 Narrative Arc
% Compile with: xelatex homecastr_pitch_v3.tex
...
\begin{tikzpicture}
  ...
  \node[font=\scriptsize, text=hcMuted] at (3.0,0.4) {One number. Today only.};
\end{tikzpicture}
\end{center}
\end{column}
...
\begin{tikzpicture}
  ...
  \node[font=\scriptsize, text=hcGold] at (3.0,0.4) {4-year range. Explainable.};
\end{tikzpicture}
\end{center}
\end{column}
\end{columns}
...
\vspace{8pt}
\begin{center}
\mutedtext{\small 150M+ residential properties. Zero affordable lot-level forecasts. Until now.}
\end{center}
\end{frame}
...
\begin{center}
\begin{tikzpicture}
  \fill[hcCard, rounded corners=8pt] (0,0) rectangle (5.5,4.5);
  \node[font=\fontsize{48}{54}\selectfont\bfseries, text=hcGold] at (2.75,3.0) {\$3.4T};
  \node[font=\large\bfseries, text=hcText] at (2.75,1.8) {SFR Market};
  \node[font=\small, text=hcMuted, align=center] at (2.75,0.8) {Institutional capital flooding in.\\Operators need forecasts to compete.};
\end{tikzpicture}
\end{center}
\end{column}
\begin{column}{0.48\textwidth}
...
\begin{tikzpicture}
  \fill[hcCard, rounded corners=8pt] (0,0) rectangle (5.5,4.5);
  \node[font=\fontsize{36}{42}\selectfont\bfseries, text=hcGold, align=center] at (2.75,3.2) {Diffusion\\Models};
  \node[font=\large\bfseries, text=hcText] at (2.75,1.8) {AI Inflection};
  \node[font=\small, text=hcMuted, align=center] at (2.75,0.8) {Foundation models now generate\\probabilistic futures, not point guesses.};
\end{tikzpicture}
\end{center}
\end{column}
\end{columns}
...
\vspace{16pt}
\begin{center}
\goldtext{\large The tools are finally here. The market is ready. Nobody has built this yet.}
\end{center}
\end{frame}
...
\begin{tikzpicture}[scale=0.9]
  ...
  \node[font=\footnotesize, text=hcMuted, anchor=east, rotate=90] at (-0.2,-2.0) {Enterprise};
  \node[font=\footnotesize, text=hcGold, anchor=east, rotate=90] at (-0.2,2.5) {Self-Serve};

  % Quadrant shading — golden quadrant (top-right)
  \fill[hcGoldLight, opacity=0.15] (0.1,0.1) rectangle (4.5,3.5);

  % Competitors
  \fill[hcMuted] (-3.0,-2.0) circle (4pt);
  \node[font=\footnotesize\bfseries, text=hcMuted, anchor=south west] at (-2.8,-1.9) {CoreLogic};

  \fill[hcMuted] (-1.5,-1.2) circle (4pt);
  \node[font=\footnotesize\bfseries, text=hcMuted, anchor=south west] at (-1.3,-1.1) {HouseCanary};

  \fill[hcMuted] (-2.5,1.8) circle (4pt);
  \node[font=\footnotesize\bfseries, text=hcMuted, anchor=south west] at (-2.3,1.9) {PropStream};

  \fill[hcMuted] (-3.5,2.2) circle (4pt);
  \node[font=\footnotesize\bfseries, text=hcMuted, anchor=south west] at (-3.3,2.3) {Zillow};
  ...
  % Quadrant label
  \node[font=\scriptsize, text=hcGold, anchor=north east] at (4.3,3.3) {\textit{white space}};
\end{tikzpicture}
\end{center}

\vspace{4pt}
\begin{center}
\goldtext{\textbf{Only self-serve, forward-looking platform.}}\\
\mutedtext{\small Everyone else looks backward, and every forward-looking play is enterprise-locked.}
\end{center}
\end{frame}
...
\begin{center}
{\fontsize{56}{64}\selectfont\bfseries\color{hcGold} 8×}\\[4pt]
{\Large LTV : CAC}\\[4pt]
\mutedtext{\small \$1,188 LTV · \$150 CAC · 1.5 month payback}
\end{center}
...
\begin{center}
\begin{tikzpicture}
  \fill[hcCard, rounded corners=6pt] (0,0) rectangle (5,2.2);
  \node[font=\large\bfseries, text=hcText] at (2.5,1.7) {Free};
  \node[font=\small, text=hcMuted, align=center] at (2.5,0.8) {Dashboard + basic forecasts\\50K+ users → lead gen funnel};
\end{tikzpicture}
\end{center}
\end{column}
...
\begin{tikzpicture}
  \fill[hcCard, rounded corners=6pt] (0,0) rectangle (5,2.2);
  \node[font=\large\bfseries, text=hcGold] at (2.5,1.7) {\$99/mo Operator};
  \node[font=\small, text=hcMuted, align=center] at (2.5,0.8) {Bulk forecasts + API access\\1,700 subs → \$2M ARR target};
\end{tikzpicture}
\end{center}
\end{column}
\end{columns}
...
\begin{itemize}\setlength{\itemsep}{6pt}
  \item \textbf{ML Engineer}\\\mutedtext{Transfer learning → 5 new metros}
...
\end{itemize}
\vspace{8pt}
\mutedtext{\scriptsize These milestones set up a strong Series~A.}
\end{column}
\end{columns}
