...
\end{tikzpicture}
\end{center}
\slidetagline{Public data in. Forecast ranges out.}
\end{frame}
...
\begin{tikzpicture}
  ...
  \node[anchor=north, text width=5.2cm, align=center] at (3.1,1.9) {
    {\scriptsize\color{hcMuted}
      A 300bps swing repriced every market overnight.\par\smallskip
      Buyers need scenario-based forecasts,\par
      not static point estimates.}
  };
  ...
  \node[anchor=north, text width=5.2cm, align=center] at (9.9,1.9) {
    {\scriptsize\color{hcMuted}
      GPU costs dropped 10{\texttimes}.\par\smallskip
      Training on 1M+ parcels is now\par
      practical — and improving fast.}
  };
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}
  ...
  \node[anchor=center] at (3.4,2.3) {\includegraphics[width=6.6cm,height=4.4cm,keepaspectratio]{assets/dashboard_screenshot.png}};
\end{tikzpicture}
\end{column}
\begin{column}{0.44\textwidth}
\centering
...
\begin{tikzpicture}
  \node[circle, inner sep=0pt, minimum size=2.4cm, path picture={
    \node at (path picture bounding box.center) {\includegraphics[width=2.4cm]{assets/dhl.jpg}};
  }] {};
\end{tikzpicture}\\[6pt]
...
\mutedtext{\scriptsize Highest quality terrain in Texas}
\end{column}
\begin{column}{0.3\textwidth}
\centering
\textbf{Sr. Data Scientist, TACC}\\
\mutedtext{\scriptsize Principal on \$40M resiliency project}
\end{column}
\begin{column}{0.3\textwidth}
\centering
\textbf{Scientific ML}\\
\mutedtext{\scriptsize Bagnold Medal Research Contributor}
\end{column}
\end{columns}
\end{frame}
