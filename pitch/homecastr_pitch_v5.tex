...
\begin{tikzpicture}
  ...
  \node[font=\fontsize{30}{36}\selectfont\bfseries, text=hcGold, align=center] at (3.1,4.1) {150M+\\Parcels Online};
  \node[font=\large\bfseries, text=hcText] at (3.1,2.7) {Data Inflection};
  \node[font=\scriptsize, text=hcMuted, align=center] at (3.1,1.4) {County appraisal records are now\\digitized and bulk-accessible at scale.};
\end{tikzpicture}
\end{center}
\end{column}
\begin{column}{0.48\textwidth}
...
\begin{tikzpicture}
  ...
  \node[font=\large\bfseries, text=hcText] at (3.1,2.7) {Now Feasible at Scale};
...
\begin{tikzpicture}
  \node[circle, inner sep=0pt, minimum size=2.5cm, path picture={
    \node at (path picture bounding box.center) {\includegraphics[width=2.5cm]{assets/dhl.jpg}};
  }] {};
\end{tikzpicture}
\end{column}
