...
\includegraphics[height=1.4cm]{homecastr-logo-cropped.png}%
}
...
\begin{tikzpicture}
  ...
  \node[font=\fontsize{40}{46}\selectfont\bfseries, text=hcGold] at (3.1,3.8) {7\%};
  \node[font=\large\bfseries, text=hcText] at (3.1,2.7) {Rate Uncertainty};
  \node[font=\scriptsize, text=hcMuted, align=center] at (3.1,1.4) {A decade of low rates is over.\\Operators need forward-looking tools.};
\end{tikzpicture}
\end{center}
\end{column}
\begin{column}{0.48\textwidth}
...
\begin{tikzpicture}
  ...
  \node[font=\large\bfseries, text=hcText] at (3.1,2.5) {Now Feasible at Scale};
  \node[font=\scriptsize, text=hcMuted, align=center] at (3.1,1.4) {Property-level probabilistic forecasts\\became computationally practical in 2023.};
\end{tikzpicture}
\end{center}
\end{column}
\end{columns}
...
\vspace{16pt}
\begin{center}
\mutedtext{\large The demand exists. The technology just caught up.}
\end{center}
\end{frame}
...
\textsuperscript{2}SFR analytics segment: \href{https://www.corelogic.com}{CoreLogic} SFR div., \href{https://www.housecanary.com}{HouseCanary} (\$18M), \href{https://www.attomdata.com}{ATTOM} (\$28M), \href{https://www.propstream.com}{PropStream} (\$25M+).\par
\textsuperscript{3}Bottom-up: {\raise.17ex\hbox{$\scriptstyle\sim$}}6,000 ICP entities across 5 metros {\texttimes} \$99/mo {\texttimes} 28\% capture. Year 1 target.}}
\end{center}
\end{frame}
...
\begin{tikzpicture}[scale=0.9]
  ...
  % Axis labels
  \node[font=\footnotesize, text=hcMuted, anchor=north] at (-3.0,-0.15) {Heuristic};
  \node[font=\footnotesize, text=hcGold, anchor=north] at (3.0,-0.15) {ML-Backtested};
  \node[font=\footnotesize, text=hcMuted, anchor=east, rotate=90] at (-0.2,-2.5) {Institutional};
  \node[font=\footnotesize, text=hcGold, anchor=east, rotate=90] at (-0.2,3.2) {Self-Serve};

  % Quadrant shading — golden quadrant (top-right)
  \fill[hcGoldLight, opacity=0.15] (0.1,0.1) rectangle (4.5,3.5);

  % CoreLogic: ML but institutional
  \fill[hcRed!60] (2.0,-2.0) circle (4pt);
  \node[font=\footnotesize\bfseries, text=hcMuted, anchor=north west] at (2.2,-2.1) {CoreLogic};

  % HouseCanary: ML, semi-self-serve, institutional UX
  \fill[hcRed!60] (1.5,-0.5) circle (4pt);
  \node[font=\footnotesize\bfseries, text=hcMuted, anchor=north west] at (1.7,-0.6) {HouseCanary};

  % PropStream: heuristic, self-serve
  \fill[hcRed!60] (-2.5,1.5) circle (4pt);
  \node[font=\footnotesize\bfseries, text=hcMuted, anchor=south east] at (-2.7,1.6) {PropStream};

  % Zillow: heuristic, consumer
  \fill[hcRed!60] (-3.0,2.5) circle (4pt);
  \node[font=\footnotesize\bfseries, text=hcMuted, anchor=east] at (-3.2,2.5) {Zillow};

  % Reventure: heuristic (ratio-based), self-serve
  \fill[hcRed!60] (-1.5,2.0) circle (4pt);
  \node[font=\footnotesize\bfseries, text=hcMuted, anchor=north] at (-1.5,1.8) {Reventure};
...
\vspace{4pt}
\begin{center}
\goldtext{\textbf{Only self-serve, ML-backtested forecast platform.}}\\
\mutedtext{\small Reventure uses heuristic ratios. CoreLogic/HouseCanary require enterprise contracts.}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  ...
  \node[font=\small, text=hcMuted, align=center] at (3.25,1.6) {Dashboard + basic forecasts\\Capture demand, build audience};
\end{tikzpicture}
\end{center}
\end{column}
...
\begin{tikzpicture}
  \fill[hcCard, rounded corners=6pt] (0,0) rectangle (6.5,3.4);
  \node[font=\large\bfseries, text=hcGold] at (3.25,2.9) {\$99/mo Operator};
  \node[font=\small, text=hcMuted, align=center] at (3.25,1.8) {Bulk forecasts + API access\\Target: 500 subs $\rightarrow$ \$600K ARR};
  \node[font=\scriptsize, text=hcMuted, align=center] at (3.25,0.5) {Year 2+: \$499/mo Pro tier\\for 50+ property portfolios};
\end{tikzpicture}
\end{center}
\end{column}
\end{columns}
...
\begin{itemize}\setlength{\itemsep}{6pt}
  ...
  \item \textbf{GTM / Sales}\\\mutedtext{Scale to 500+ Operator subs}
