...
\begin{tikzpicture}
  ...
  \node[font=\scriptsize, text=hcMuted, align=center] at (3.1,1.4) {Operators manage billions in assets\\without precomputed, backtested forecasts.};
\end{tikzpicture}
\end{center}
\end{column}
\begin{column}{0.48\textwidth}
...
\begin{tikzpicture}
  ...
  \node[font=\large\bfseries, text=hcText] at (3.1,2.5) {Beyond Pro Formas};
  \node[font=\scriptsize, text=hcMuted, align=center] at (3.1,1.4) {Outperforms traditional pro forma.\\Backtested across 4 years of actuals.};
\end{tikzpicture}
\end{center}
\end{column}
\end{columns}
...
\begin{tikzpicture}[scale=0.9]
  ...
  \node[font=\footnotesize, text=hcGold, anchor=east, rotate=90] at (-0.2,3.2) {Consumer};
  ...
  \node[font=\footnotesize\bfseries, text=hcMuted, anchor=north west] at (1.7,-2.1) {CoreLogic};

  % HouseCanary: forward-looking, self-serve, but institutional UX
  \fill[hcRed!60] (2.5,-0.8) circle (4pt);
  \node[font=\footnotesize\bfseries, text=hcMuted, anchor=north west] at (2.7,-0.9) {HouseCanary};

  % PropStream: backward-looking, consumer-ish
  \fill[hcRed!60] (-2.5,1.0) circle (4pt);
  \node[font=\footnotesize\bfseries, text=hcMuted, anchor=south east] at (-2.7,1.1) {PropStream};

  % Zillow: backward-looking, consumer
  \fill[hcRed!60] (-3.5,2.5) circle (4pt);
  \node[font=\footnotesize\bfseries, text=hcMuted, anchor=west] at (-3.3,2.5) {Zillow};

  % Reventure App: trend visualization (ZIP level), consumer
...
\vspace{4pt}
\begin{center}
\goldtext{\textbf{Only consumer-facing, property-level forecast platform.}}\\
\mutedtext{\small Reventure shows ZIP-code trends. HouseCanary/CoreLogic forecast for analysts, not consumers.}
\end{center}
\end{frame}
