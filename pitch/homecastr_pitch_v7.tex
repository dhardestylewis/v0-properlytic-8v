...
\centering
\statbox{15\%}{median error (p50)}\\[14pt]
\statbox{4yr}{forecast horizon}\\[14pt]
\statbox{1M+}{lots forecasted}
\end{column}
\end{columns}
...
\begin{center}
\begin{tikzpicture}
  \fill[hcCard, rounded corners=8pt] (0,0) rectangle (6.8,5.0);
  \node[anchor=north, text width=6.0cm, align=center] at (3.4,4.7) {
    {\fontsize{28}{34}\selectfont\bfseries\color{hcGold} 300bps}
  };
  \node[anchor=north, text width=6.0cm, align=center] at (3.4,2.9) {
    {\large\bfseries\color{hcText} rate swing}
  };
  \node[anchor=north, text width=5.6cm, align=center] at (3.4,2.1) {
    {\scriptsize\color{hcMuted}
      Rates surged from 3\% to 7\%+\\[3pt]
      in under two years,\\[3pt]
      repricing every market.}
  };
\end{tikzpicture}
\end{center}
\end{column}
...
\begin{tikzpicture}
  \fill[hcCard, rounded corners=8pt] (0,0) rectangle (6.8,5.0);
  \node[anchor=north, text width=6.0cm, align=center] at (3.4,4.7) {
    {\fontsize{22}{28}\selectfont\bfseries\color{hcGold} ML Forecasting}
  };
  \node[anchor=north, text width=6.0cm, align=center] at (3.4,2.9) {
  ...
  \node[anchor=north, text width=5.6cm, align=center] at (3.4,2.1) {
    {\scriptsize\color{hcMuted}
      GPU costs dropped 10{\texttimes}.\\[3pt]
      Training at 100M+ parcel scale\\[3pt]
      is now within reach.}
  };
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.9]
  ...
  % CoreLogic (was: text label)
  \node[inner sep=2pt, fill=white, rounded corners=2pt] at (2.0,-2.0)
    {\includegraphics[height=0.45cm]{assets/logo_corelogic.png}};

  % HouseCanary
  \node[inner sep=2pt, fill=white, rounded corners=2pt] at (1.5,-0.8)
    {\includegraphics[height=0.42cm]{assets/logo_housecanary.png}};

  % PropStream
  \node[inner sep=2pt, fill=white, rounded corners=2pt] at (-2.5,1.5)
    {\includegraphics[height=0.42cm]{assets/logo_propstream.png}};

  % Zillow
  \node[inner sep=2pt, fill=white, rounded corners=2pt] at (-3.0,2.5)
    {\includegraphics[height=0.42cm]{assets/logo_zillow_v2.png}};

  % Reventure
  \node[inner sep=2pt, fill=white, rounded corners=2pt] at (-0.8,2.0)
    {\includegraphics[height=0.40cm]{assets/logo_reventure.png}};
...
\vspace{4pt}
\begin{center}
{\small\color{hcMuted}\textit{Only product-led platform with backtested, probabilistic forecasts.}}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  ...
  \node[font=\small, text=hcMuted, align=center] at (3.25,1.8) {Dashboard \& basic forecasts\\[10pt]Builds audience, captures intent};
\end{tikzpicture}
\end{center}
\end{column}
...
\begin{tikzpicture}
  ...
  \node[font=\small, text=hcMuted, align=center] at (3.25,1.8) {Bulk forecasts \& API access\\[10pt]Full portfolio analytics};
\end{tikzpicture}
\end{center}
\end{column}
\end{columns}
...
\begin{tikzpicture}[
  ...
  \node[block] (signal) at (3.0,1.8) {\textbf{Chat signals}\\{\scriptsize reveal buyer intent}};
...
{\small\color{hcMuted} Direct calls to Houston\\SFR operators; pilots close\\word-of-mouth referrals.}
\end{center}
\end{column}
\begin{column}{0.32\textwidth}
\begin{center}
...
\centering
{\includegraphics[height=1.1cm]{assets/logo_summit.png}}\\[4pt]
\textbf{Summit Geospatial}\\
\mutedtext{\scriptsize Highest-quality terrain, Texas}
\end{column}
\begin{column}{0.33\textwidth}
\centering
{\colorbox{hcCard}{\includegraphics[height=0.8cm]{assets/logo_tacc.png}}}\\[4pt]
\textbf{Data Scientist, TACC}\\
\mutedtext{\scriptsize Lead on \$40M resiliency project}
\end{column}
\begin{column}{0.33\textwidth}
\centering
{\includegraphics[height=0.8cm]{assets/logo_egu.png}}\\[4pt]
\textbf{Scientific ML}\\
