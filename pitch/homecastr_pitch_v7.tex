...
\begin{tikzpicture}
  ...
  \node[align=center, text width=6.0cm] at (3.4,2.5) {
    {\fontsize{28}{34}\selectfont\bfseries\color{hcGold} 300bps}\\[8pt]
    {\large\bfseries\color{hcText} in our training data}\\[10pt]
    {\scriptsize\color{hcMuted}
      Our model trained through rates surging\\[2pt]
      from 3\% to 7\%+. It learned to\\[2pt]
      price rate cycles. Others haven't.}
  };
\end{tikzpicture}
\end{center}
\end{column}
...
\begin{tikzpicture}
  \fill[hcCard, rounded corners=8pt] (0,0) rectangle (6.8,5.0);
  \node[align=center, text width=6.0cm] at (3.4,2.5) {
    {\fontsize{22}{28}\selectfont\bfseries\color{hcGold} ML Forecasting}\\[8pt]
    {\large\bfseries\color{hcText} now feasible at scale}\\[10pt]
    {\scriptsize\color{hcMuted}
      GPU costs dropped 10{\texttimes}.\\[2pt]
      Training at 100M+ property scale\\[2pt]
      is now within reach.}
  };
\end{tikzpicture}
\end{center}
