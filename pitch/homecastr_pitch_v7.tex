...
\centering
\statbox{15\%}{median error}\\[14pt]
\statbox{4yr}{forecast horizon}\\[14pt]
\statbox{1M+}{properties forecasted}
\end{column}
\end{columns}

\slidetagline{\href{https://homecastr.com}{\textcolor{hcGold}{homecastr.com}} \textbar{} Houston metro. Live. Free. {\tiny $^{*}$50\% of properties fall below this error.}}
\end{frame}
...
\begin{tikzpicture}
  ...
  \node[anchor=north, text width=6.0cm, align=center] at (3.4,2.9) {
    {\large\bfseries\color{hcText} in our training data}
  };
  \node[anchor=north, text width=5.6cm, align=center] at (3.4,2.1) {
    {\scriptsize\color{hcMuted}
      Our model trained through rates\\[3pt]
      surging from 3\% to 7\%+.\\[3pt]
      It learned to price rate cycles.\\[3pt]
      Others haven't.}
  };
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.9]
  ...
  \node[inner sep=1pt] at (2.0,-2.0)
    {\includegraphics[height=0.45cm]{assets/logo_corelogic.png}};

  % HouseCanary
  \node[inner sep=1pt] at (1.5,-0.8)
    {\includegraphics[height=0.42cm]{assets/logo_housecanary.png}};

  % PropStream
  \node[inner sep=1pt] at (-3.8,1.0)
    {\includegraphics[height=0.42cm]{assets/logo_propstream.png}};

  % Zillow
  \node[inner sep=1pt] at (-3.2,2.8)
    {\includegraphics[height=0.42cm]{assets/logo_zillow_v2.png}};

  % Reventure (white logo — needs dark bg)
  \node[inner sep=2pt, fill=hcCard, rounded corners=2pt] at (-2.5,2.2)
    {\includegraphics[height=0.40cm]{assets/logo_reventure.png}};

  % Homecastr — golden quadrant (logo replaces text label)
  \node[anchor=south west] at (3.2,2.3) {\includegraphics[height=0.55cm]{assets/homecastr-logo-horizontal.png}};
