-- =============================================================================
-- UNSD aggregate rebuild — optimized for Supabase SQL Editor (<60s/query)
...
(unsd_geoid, origin_year, horizon_m, forecast_year,
...
SELECT pl.unsd_geoid, mp.origin_year, mp.horizon_m, mp.forecast_year,
...
WHERE mp.series_kind = 'forecast' AND mp.variant_id = '__forecast__'
  AND mp.horizon_m = 12 AND pl.unsd_geoid IS NOT NULL
...
JOIN public.parcel_ladder_v1 pl USING (acct)
WHERE mp.series_kind = 'forecast' AND mp.variant_id = '__forecast__'
  AND mp.horizon_m = 24 AND pl.unsd_geoid IS NOT NULL
GROUP BY pl.unsd_geoid, mp.origin_year, mp.horizon_m, mp.forecast_year;
...
JOIN public.parcel_ladder_v1 pl USING (acct)
WHERE mp.series_kind = 'forecast' AND mp.variant_id = '__forecast__'
  AND mp.horizon_m = 36 AND pl.unsd_geoid IS NOT NULL
GROUP BY pl.unsd_geoid, mp.origin_year, mp.horizon_m, mp.forecast_year;
...
JOIN public.parcel_ladder_v1 pl USING (acct)
WHERE mp.series_kind = 'forecast' AND mp.variant_id = '__forecast__'
  AND mp.horizon_m = 48 AND pl.unsd_geoid IS NOT NULL
GROUP BY pl.unsd_geoid, mp.origin_year, mp.horizon_m, mp.forecast_year;
...
JOIN public.parcel_ladder_v1 pl USING (acct)
WHERE mp.series_kind = 'forecast' AND mp.variant_id = '__forecast__'
  AND mp.horizon_m = 60 AND pl.unsd_geoid IS NOT NULL
GROUP BY pl.unsd_geoid, mp.origin_year, mp.horizon_m, mp.forecast_year;
