-- =============================================================================
-- NEIGHBORHOOD aggregate rebuild — optimized for Supabase SQL Editor (<60s/query)
...
(neighborhood_id, origin_year, horizon_m, forecast_year,
...
SELECT pl.neighborhood_id, mp.origin_year, mp.horizon_m, mp.forecast_year,
...
WHERE mp.series_kind = 'forecast' AND mp.variant_id = '__forecast__'
  AND mp.horizon_m = 12 AND pl.neighborhood_id IS NOT NULL
...
JOIN public.parcel_ladder_v1 pl USING (acct)
WHERE mp.series_kind = 'forecast' AND mp.variant_id = '__forecast__'
  AND mp.horizon_m = 24 AND pl.neighborhood_id IS NOT NULL
GROUP BY pl.neighborhood_id, mp.origin_year, mp.horizon_m, mp.forecast_year;
...
JOIN public.parcel_ladder_v1 pl USING (acct)
WHERE mp.series_kind = 'forecast' AND mp.variant_id = '__forecast__'
  AND mp.horizon_m = 36 AND pl.neighborhood_id IS NOT NULL
GROUP BY pl.neighborhood_id, mp.origin_year, mp.horizon_m, mp.forecast_year;
...
JOIN public.parcel_ladder_v1 pl USING (acct)
WHERE mp.series_kind = 'forecast' AND mp.variant_id = '__forecast__'
  AND mp.horizon_m = 48 AND pl.neighborhood_id IS NOT NULL
GROUP BY pl.neighborhood_id, mp.origin_year, mp.horizon_m, mp.forecast_year;
...
JOIN public.parcel_ladder_v1 pl USING (acct)
WHERE mp.series_kind = 'forecast' AND mp.variant_id = '__forecast__'
  AND mp.horizon_m = 60 AND pl.neighborhood_id IS NOT NULL
GROUP BY pl.neighborhood_id, mp.origin_year, mp.horizon_m, mp.forecast_year;
