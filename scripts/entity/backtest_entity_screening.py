...
After worldmodel.py runs in Cell 1, this cell:
  ...
  3) For each origin: loads checkpoint, runs inference,
     applies track-record filter, computes:
       ...
       - VALUE-BRACKET ANALYSIS: per-bracket MAE, coverage, growth direction
         (tests hypothesis that $1M+ properties are OOD due to undersampling)
...
# ─── Value-bracket analysis (tests $1M+ undersampling hypothesis) ───
# Brackets defined as (label, lo_dollar, hi_dollar)
VALUE_BRACKETS = [
    ("<200K",    0,       200_000),
...
bracket_rows = []         # {origin, horizon, bracket, n, med_growth, pct_neg, mae, coverage, rho}
...
for ORIGIN in BACKTEST_ORIGINS:
    ...
    for hold in [1, 2, 3, 4, 5]:
        ...
        # ══════════════════════════════════════════════════════════
        # VALUE-BRACKET ANALYSIS: tests $1M+ undersampling hypothesis
        # For each bracket: MAE, fan coverage, % negative growth, ρ
        # ══════════════════════════════════════════════════════════
        for bkt_label, bkt_lo, bkt_hi in VALUE_BRACKETS:
            ...
            for i in range(len(bt_accts)):
                ...
                v = base_v.get(acct, 0)
                if bkt_lo <= v < bkt_hi:
                    bkt_indices.append(i)

            bkt_n = len(bkt_indices)
            if bkt_n < 5:
                continue

            # Predicted growth for this bracket at this horizon
            h_log = np.nanmedian(y_levels[:, :, hold-1], axis=1)
            h_growth = np.expm1(h_log - ya) * 100

            bkt_pred = np.array([h_growth[i] for i in bkt_indices])

            # Actual growth for this bracket
            bkt_actual = []
            bkt_pred_matched = []
            bkt_fan_hits = 0
            bkt_fan_checks = 0
            for i in bkt_indices:
                acct = str(bt_accts[i]).strip()
                bv = base_v.get(acct, 0)
                av_dollar = fv.get(acct)
                if bv > 0 and av_dollar is not None:
                    actual_g = (av_dollar - bv) / bv * 100
                    bkt_actual.append(actual_g)
                    bkt_pred_matched.append(h_growth[i])
                    # Fan coverage: did P10–P90 cover the actual dollar value?
                    h_levels_i = y_levels[i, :, hold-1]
                    p10_d = np.exp(np.nanpercentile(h_levels_i, 10))
                    p90_d = np.exp(np.nanpercentile(h_levels_i, 90))
                    bkt_fan_checks += 1
                    if p10_d <= av_dollar <= p90_d:
                        bkt_fan_hits += 1
            ...
            coverage = (bkt_fan_hits / bkt_fan_checks * 100) if bkt_fan_checks > 0 else None
            bkt_rho = None
            if len(bkt_actual) > 10:
                bkt_rho, _ = spearmanr(bkt_pred_matched, bkt_actual)
                bkt_rho = float(bkt_rho)

            bracket_rows.append({
                "origin": ORIGIN, "horizon": hold, "bracket": bkt_label,
                "n": bkt_n, "n_with_actual": len(bkt_actual),
                "med_growth": med_growth, "pct_neg": pct_neg,
                "mae": mae, "coverage": coverage, "rho": bkt_rho,
            })
...
# ══════════════════════════════════════════════════════════════════
# VALUE-BRACKET ANALYSIS TABLE (tests $1M+ undersampling hypothesis)
# ══════════════════════════════════════════════════════════════════
if bracket_rows:
    print("\n" + "=" * 105)
    print("🏠 VALUE-BRACKET ANALYSIS: Does the model underperform on high-value properties?")
    print("   Hypothesis: $1M+ parcels are OOD due to <5% representation in 20K training samples")
    print("   Symptoms: negative growth bias, poor coverage, low ρ in $1M+ bracket")
    print("=" * 105)

    # Aggregate by bracket across all origins/horizons
    bkt_agg = defaultdict(lambda: {"med_growths": [], "pct_negs": [], "maes": [],
                                    "coverages": [], "rhos": [], "n_total": 0, "rows": 0})
    for br in bracket_rows:
        ba = bkt_agg[br["bracket"]]
        ba["med_growths"].append(br["med_growth"])
        ba["pct_negs"].append(br["pct_neg"])
        if br["mae"] is not None: ba["maes"].append(br["mae"])
        if br["coverage"] is not None: ba["coverages"].append(br["coverage"])
        if br["rho"] is not None: ba["rhos"].append(br["rho"])
        ba["n_total"] += br["n"]
        ba["rows"] += 1

    print(f"\n  {'Bracket':<12} {'Avg n':>7} {'Med Grw':>8} {'%Neg':>6} {'MAE':>7} {'Covg':>6} {'ρ':>7}  Diagnosis")
    print(f"  {'─'*12} {'─'*7} {'─'*8} {'─'*6} {'─'*7} {'─'*6} {'─'*7}  {'─'*25}")

    for bkt_label, _, _ in VALUE_BRACKETS:
        ba = bkt_agg.get(bkt_label)
        if ba is None or ba["rows"] == 0:
            continue
        avg_n = ba["n_total"] / ba["rows"]
        avg_mg = np.mean(ba["med_growths"])
        avg_neg = np.mean(ba["pct_negs"])
        avg_mae = np.mean(ba["maes"]) if ba["maes"] else float("nan")
        avg_cov = np.mean(ba["coverages"]) if ba["coverages"] else float("nan")
        avg_rho = np.mean(ba["rhos"]) if ba["rhos"] else float("nan")

        # Diagnosis
        diag = ""
        if bkt_label == "1M+":
            issues = []
            if avg_neg > 50: issues.append("neg-bias")
            if not np.isnan(avg_cov) and avg_cov < 60: issues.append("poor-coverage")
            if not np.isnan(avg_rho) and avg_rho < 0.1: issues.append("low-ρ")
            if issues:
                diag = f"⚠️ OOD signal: {', '.join(issues)}"
            else:
                diag = "✅ No OOD signal"
        else:
            diag = "— baseline"

        mae_s = f"{avg_mae:>6.1f}%" if not np.isnan(avg_mae) else "    —  "
        cov_s = f"{avg_cov:>5.1f}%" if not np.isnan(avg_cov) else "   —  "
        rho_s = f"{avg_rho:>6.3f}" if not np.isnan(avg_rho) else "   —  "
        print(f"  {bkt_label:<12} {avg_n:>6.0f}  {avg_mg:>+7.1f}% {avg_neg:>5.1f}% {mae_s} {cov_s} {rho_s}  {diag}")

    # Detailed per-origin view for $1M+ bracket
    hv_rows = [r for r in bracket_rows if r["bracket"] == "1M+"]
    if hv_rows:
        print(f"\n  ── $1M+ bracket detail (per origin × horizon) ──")
        print(f"  {'Origin':>6} {'Hold':>4}  {'n':>6} {'Med Grw':>8} {'%Neg':>6} {'MAE':>7} {'Covg':>6} {'ρ':>7}")
        print(f"  {'─'*6} {'─'*4}  {'─'*6} {'─'*8} {'─'*6} {'─'*7} {'─'*6} {'─'*7}")
        for r in sorted(hv_rows, key=lambda x: (x["origin"], x["horizon"])):
            mae_s = f"{r['mae']:>6.1f}%" if r['mae'] is not None else "    —  "
            cov_s = f"{r['coverage']:>5.1f}%" if r['coverage'] is not None else "   —  "
            rho_s = f"{r['rho']:>6.3f}" if r['rho'] is not None else "   —  "
            flag = "⚠️" if r["pct_neg"] > 50 else "  "
            print(f"  {r['origin']:>6} {r['horizon']:>4}yr  {r['n']:>6} {r['med_growth']:>+7.1f}% "
                  f"{r['pct_neg']:>5.1f}% {mae_s} {cov_s} {rho_s} {flag}")

        # Compare $1M+ vs <$1M
        hv_neg = np.mean([r["pct_neg"] for r in hv_rows])
        lv_rows = [r for r in bracket_rows if r["bracket"] != "1M+"]
        lv_neg = np.mean([r["pct_neg"] for r in lv_rows]) if lv_rows else 0
        hv_cov = np.mean([r["coverage"] for r in hv_rows if r["coverage"] is not None])
        lv_cov = np.mean([r["coverage"] for r in lv_rows if r["coverage"] is not None]) if lv_rows else 0

        print(f"\n  ── Hypothesis test summary ──")
        print(f"     $1M+ avg %negative: {hv_neg:.1f}%  |  <$1M avg %negative: {lv_neg:.1f}%")
        if not np.isnan(hv_cov) and not np.isnan(lv_cov):
            print(f"     $1M+ avg coverage:  {hv_cov:.1f}%  |  <$1M avg coverage:  {lv_cov:.1f}%")
        if hv_neg > lv_neg + 15:
            print(f"     → CONFIRMED: $1M+ properties show {hv_neg-lv_neg:.0f}pp more negative growth predictions")
            print(f"       This is consistent with OOD mean-reversion from undersampling")
        elif hv_neg > lv_neg + 5:
            print(f"     → PARTIAL: $1M+ shows {hv_neg-lv_neg:.0f}pp more negative — mild signal")
        else:
            print(f"     → NOT CONFIRMED: $1M+ negative rate similar to rest of market")
