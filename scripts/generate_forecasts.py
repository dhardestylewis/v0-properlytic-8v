...
# Determine which columns to load by reading just the header
header = pd.read_csv(csv_path, nrows=0).columns.tolist()
desired_cols = ['year', 'protest', 'standardized_tcad_id', 'ears_matched', 'ears_source'] + NUMERIC_FEATURES + CATEGORICAL_FEATURES
...
X_score = df.loc[score_mask, feature_cols].values.astype(np.float32)
ids_score = df.loc[score_mask, 'standardized_tcad_id'].values

scaler = StandardScaler(copy=False)
X_train_s = scaler.fit_transform(X_train)
np.nan_to_num(X_train_s, nan=0.0, copy=False)

X_score_s = scaler.transform(X_score)
np.nan_to_num(X_score_s, nan=0.0, copy=False)
...
# We predict identical scores for multiple forecast horizons (no temporal leakage, frozen features)
FORECAST_YEARS = [2025, 2026, 2027]

print(f"Uploading {len(X_score_s)} scored records for horizons {FORECAST_YEARS} to Supabase...")
batch_size = 5000

for fc_year in FORECAST_YEARS:
    print(f"\n--- Uploading Forecast Year {fc_year} ---")
    out_df = pd.DataFrame({
    ...
    for i in range(0, len(records), batch_size):
        ...
        if res.status_code != 200:
            print(f'Error at batch {i} for year {fc_year}:', res.status_code, res.text)
            break
        else:
            print(f'  Inserted {i+len(batch)} / {len(records)}')

print('Done uploading all OOT forecast horizons!')
