...
> **CRITICAL #2 - DATA SAFETY**: NEVER delete directories or files after a move/copy operation without explicitly verifying using a checksum or file count that the destination exists and contains the correct data. Use `robocopy` or `rsync` with extreme care and checking exit codes. When in doubt, rename the source folder to `_backup` or `_old` instead of deleting it. [Added: 2026-01-26 21:03]
