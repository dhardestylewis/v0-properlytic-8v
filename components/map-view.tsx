...
export function MapView({
  ...
  useEffect(() => {
    ...
    fetchTimeoutRef.current = setTimeout(() => {
      ...
      if (currentH3Res !== lastResolutionRef.current) {
        console.log(`[RES] Resolution changed: ${lastResolutionRef.current} → ${currentH3Res}, clearing old data`)
        setRealHexData([])
        lastResolutionRef.current = currentH3Res
      }
      ...
      getH3DataForResolution(currentH3Res, year, bounds)
        .then((data) => {
          ...
          console.log(
            `[DIAG] ✓ COMPLETE FETCH - Viewport: lat=${bounds.minLat.toFixed(2)}→${bounds.maxLat.toFixed(2)}, lng=${bounds.minLng.toFixed(2)}→${bounds.maxLng.toFixed(2)}`,
          )
          console.log(
      ...
      {(() => {
        ...
        return (
          <div className="absolute bottom-4 left-4 bg-card/95 backdrop-blur-sm border border-border rounded-lg px-3 py-2 text-xs text-muted-foreground z-40 shadow-lg font-mono">
            ...
            <div className="text-[10px] opacity-70">
              Lng: {bounds.minLng.toFixed(2)}→{bounds.maxLng.toFixed(2)}
            </div>
            {realHexData.length >= 9900 && <div className="text-amber-400 font-semibold">⚠ LIMIT HIT</div>}
          </div>
        )
      })()}
    </div>
